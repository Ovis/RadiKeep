...
export const initExternalImport = (verificationToken: string, showToast: ShowToastFn): void => {
    const panel = document.getElementById('setting-panel-external-import') as HTMLDivElement | null;
    if (!panel) {
        return;
    }

    const scanButton = document.getElementById('external-import-scan-btn') as HTMLButtonElement | null;
    const applyDefaultTagCheckbox = document.getElementById('external-import-apply-default-tag') as HTMLInputElement | null;
    const markAsListenedCheckbox = document.getElementById('external-import-mark-as-listened') as HTMLInputElement | null;
    const exportButton = document.getElementById('external-import-export-btn') as HTMLButtonElement | null;
    const importButton = document.getElementById('external-import-import-btn') as HTMLButtonElement | null;
    const importFileInput = document.getElementById('external-import-file-input') as HTMLInputElement | null;
    const saveButton = document.getElementById('external-import-save-btn') as HTMLButtonElement | null;
    const tableBody = document.getElementById('external-import-table-body') as HTMLTableSectionElement | null;
    const rowTemplate = document.getElementById('external-import-row-template') as HTMLTemplateElement | null;
    const paginationList = document.getElementById('external-import-pagination-list') as HTMLUListElement | null;
    const totalCountLabel = document.getElementById('external-import-total-count') as HTMLSpanElement | null;
    const selectAllCheckbox = document.getElementById('external-import-select-all') as HTMLInputElement | null;
    ...
    const renderPagination = (): void => {
        ...
        const createPageItem = (page: number, text?: string): HTMLLIElement => {
        ...
        if (currentPage === 1) {
            (prev.firstElementChild as HTMLButtonElement).disabled = true;
        }
        ...
        if (currentPage === totalPages) {
            (next.firstElementChild as HTMLButtonElement).disabled = true;
        }
    ...
    const renderTable = (): void => {
        ...
        pageItems.forEach((candidate) => {
            ...
            const select = row.querySelector('.external-import-select') as HTMLInputElement | null;
            const filePath = row.querySelector('.external-import-file-path') as HTMLInputElement | null;
            const title = row.querySelector('.external-import-title') as HTMLInputElement | null;
            const description = row.querySelector('.external-import-description') as HTMLInputElement | null;
            const stationName = row.querySelector('.external-import-station-name') as HTMLInputElement | null;
            const broadcastAt = row.querySelector('.external-import-broadcast-at') as HTMLInputElement | null;
            const tags = row.querySelector('.external-import-tags') as HTMLInputElement | null;
    ...
    const updateButtons = (disabled: boolean): void => {
